...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding M2M table for field orders on 'Product'
        m2m_table_name = db.shorten_name(u'adelieproj_product_orders')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('product', models.ForeignKey(orm[u'adelieproj.product'], null=False)),
            ('order', models.ForeignKey(orm[u'adelieproj.order'], null=False))
        ))
        db.create_unique(m2m_table_name, ['product_id', 'order_id'])

        # Deleting field 'Order.product'
        db.delete_column(u'adelieproj_order', 'product_id')


    def backwards(self, orm):
        # Removing M2M table for field orders on 'Product'
        db.delete_table(db.shorten_name(u'adelieproj_product_orders'))


        # User chose to not deal with backwards NULL issues for 'Order.product'
        raise RuntimeError("Cannot reverse this migration. 'Order.product' and its values cannot be restored.")
